  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
      ...
      <div className="bg-bar-800 p-6 rounded-xl border border-bar-700 h-fit">
        <h3 className="text-xl font-bold text-bar-text mb-4 flex items-center gap-2">
        ...
        <form onSubmit={handleCreate} className="space-y-4">
          <div>
            ...
            <input value={name} onChange={e => setName(e.target.value)} className="w-full bg-bar-900 border border-bar-600 rounded p-2 text-bar-text outline-none focus:border-bar-500" placeholder="Ej. Juan Pérez" />
          </div>
          <div>
            <label className="block text-sm font-medium text-slate-300 mb-1">Nombre de Usuario</label>
            <input value={username} onChange={e => setUsername(e.target.value)} className="w-full bg-bar-900 border border-bar-600 rounded p-2 text-bar-text outline-none focus:border-bar-500" placeholder="Ej. juanp" />
          </div>
          <div>
            <label className="block text-sm font-medium text-slate-300 mb-1">Contraseña</label>
            <input type="password" value={password} onChange={e => setPassword(e.target.value)} className="w-full bg-bar-900 border border-bar-600 rounded p-2 text-bar-text outline-none focus:border-bar-500" placeholder="••••••" />
          </div>
          <div>
            <label className="block text-sm font-medium text-slate-300 mb-1">Rol</label>
            <select value={role} onChange={e => setRole(e.target.value as Role)} className="w-full bg-bar-900 border border-bar-600 rounded p-2 text-bar-text outline-none focus:border-bar-500">
      ...
      <div className="lg:col-span-2 space-y-4">
        <h2 className="text-2xl font-bold text-bar-text">Usuarios del Sistema</h2>
          ...
          {users.map(user => (
            <div key={user.id} className="bg-bar-800 border border-bar-700 p-4 rounded-xl flex items-center justify-between">
              <div className="flex items-center gap-4">
                ...
                <div>
                  <p className="font-bold text-bar-text">{user.name}</p>
