...
import { getSessions, clearHistoricalData, getUsers, reopenSession, approveSession } from '../services/db';
    ...
    return (
        <div className="space-y-6 pb-20">
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                <div>
                    <h2 className="text-3xl font-bold text-bar-text">Historial de Turnos</h2>
            ...
            {currentUser.role === 'ADMIN' && (
                <div className="bg-bar-800 p-4 rounded-xl border border-bar-700 flex items-center gap-4">
                    ...
                    <select
                        ...
                        className="bg-bar-900 border border-bar-600 rounded-lg p-2 text-bar-text text-sm outline-none focus:border-bar-500"
                    >
                                ...
                                filteredSessions.map(s => {
                                    ...
                                    return (
                                        <tr key={s.id} className="hover:bg-bar-700/30 transition-colors">
                                            ...
                                            <td className="p-4 text-bar-text font-medium">
                                            ...
                                            <td className="p-4 flex justify-center">
                                                <button
                                                    ...
                                                    className="p-2 bg-bar-700 hover:bg-bar-600 text-bar-text rounded-lg flex items-center gap-2 transition-colors text-xs font-bold"
                                                >
                ...
                <div className="fixed inset-0 z-50 bg-black/80 backdrop-blur-sm flex items-center justify-center p-4">
                    <div className="bg-bar-800 w-full max-w-2xl rounded-2xl border border-bar-600 shadow-2xl flex flex-col max-h-[90vh]">
                        ...
                        <div className="p-6 border-b border-bar-700 flex justify-between items-start">
                            <div>
                                <h3 className="text-xl font-bold text-bar-text flex items-center gap-2">
                                    <FileText size={20} className="text-bar-500" /> Detalle de Turno
                                </h3>
                                <p className="text-slate-400 text-sm mt-1">
                                    Cerrado por <span className="text-bar-text font-medium">{usersMap[selectedSession.closedBy || '']}</span> el {new Date(selectedSession.closedAt || '').toLocaleString()}
                                </p>
                            </div>
                            <button onClick={() => setIsDetailOpen(false)} className="text-slate-400 hover:text-bar-text bg-bar-900 p-2 rounded-lg">
                                <Archive size={20} />
                            </button>
                        </div>
                        ...
                        <div className="flex-1 overflow-y-auto p-6 space-y-6">
                            ...
                            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                                <div className="bg-bar-900 p-3 rounded-lg border border-bar-700">
                                    ...
                                    <p className="text-bar-text font-mono text-lg">${selectedSession.salesReport.totalRevenue.toLocaleString()}</p>
                                </div>
                                <div className="bg-bar-900 p-3 rounded-lg border border-bar-700">
                            ...
                            <div className="bg-slate-800/50 p-4 rounded-xl border border-slate-700">
                                <h4 className="text-bar-text font-bold mb-3 flex items-center gap-2">
                                ...
                                <div className="space-y-2 text-sm">
                                    ...
                                    <div className="flex justify-between text-bar-text font-bold text-base">
                                    ...
                                    <div className="border-t border-slate-600 pt-2 flex justify-between items-center">
                                        ...
                                        <span className={`font-mono font-bold px-2 py-1 rounded ${(selectedSession.salesReport.difference || 0) < 0 ? 'bg-rose-900 text-rose-300' :
                            ...
                            {currentUser.role === 'ADMIN' ? (
                                selectedSession.status === 'PENDING_APPROVAL' ? (
                                    <div className="flex gap-3">
                                        <button onClick={() => setIsDetailOpen(false)} className="flex-1 bg-bar-700 text-bar-text py-3 rounded-lg font-bold">
                                            Cerrar
                                        </button>
                                        <button
                                            onClick={handleApprove}
                                            className="flex-1 bg-emerald-600 hover:bg-emerald-500 text-bar-text py-3 rounded-lg font-bold shadow-lg shadow-emerald-600/20"
                                        >
                                            <CheckCircle size={18} className="inline mr-2" /> Aprobar Turno
                                        </button>
                                    </div>
                                ) : (
                                    !isReopenMode ? (
                                        ...
                                        <div className="space-y-3 animate-in fade-in slide-in-from-bottom-2">
                                            ...
                                            <input
                                                ...
                                                className="w-full bg-bar-950 border border-rose-700 rounded-lg p-3 text-bar-text focus:outline-none focus:ring-1 focus:ring-rose-500"
                                            />
                                            <div className="flex gap-3">
                                                <button
                                                    onClick={() => setIsReopenMode(false)}
                                                    className="flex-1 py-3 bg-slate-800 text-slate-400 hover:text-bar-text rounded-lg font-medium"
                                                >
                                                    Cancelar
                                                </button>
                                                <button
                                                    ...
                                                    className="flex-1 py-3 bg-rose-600 hover:bg-rose-500 text-bar-text rounded-lg font-bold disabled:opacity-50"
                                                >
                                ...
                                <button onClick={() => setIsDetailOpen(false)} className="w-full bg-bar-700 text-bar-text py-3 rounded-lg font-bold">
                                    Cerrar
                                </button>
                            )}
                        </div>
